...
namespace GameServer.Services
{
    class MapService : Singleton<MapService>
    {
        ...
        private void OnMapTeleport(NetConnection<NetSession> sender, MapTeleportRequest request)
        {
            ...
            {
                Log.WarningFormat("Source TeleporterID [{0}] does not exist", request.teleporterId);
                return;
            }
            ...
            if (source.LinkTo == 0 || DataManager.Instance.Teleporters.ContainsKey(source.LinkTo))
                Log.WarningFormat("Source TeleporterID [{0}] LinkTo ID [{1}] does not exist", request.teleporterId, source.LinkTo);
